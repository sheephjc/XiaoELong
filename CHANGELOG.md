...
- `xiaoelong.cn` 仅承载 `/pet` 发布页；桌面客户端连接和自动更新继续使用现有服务器 IP。
...
- 为 HJC、HSX、mx、chui、郭、HF 与 A神加入生日祝福：生日当天首次打开左键面板会优先显示专属贺卡，关闭后当天不再重复弹出，下一年生日会再次展示。
- 生日当天在左键面板持续显示同行等大的中文祝福与 `HAPPY BIRTHDAY`，并播放不会拦截鼠标操作的彩带、蛋糕雨和升空烟花。

### Changed（调整）

- 生日贺卡关闭按钮与面板祝福改为无边框样式，调整字号、对齐和宽度，确保姓名与“生日快乐”完整显示。
- 面板普通文字不再被鼠标框选；聊天输入框等可编辑区域继续支持光标和文字选择。
- 心情选择器提升到功能面板之上；桌宠独立心情气泡出现时会临时提高原生窗口层级，避免被聊天窗口遮挡。

### Fixed（修复）

- 聊天列表在图片或其他内容改变布局后继续贴住底部；用户主动滚动、触摸或按下列表时立即取消待执行的自动滚动，避免与手动浏览争抢位置。

### Notes（说明）

- 本版本没有新增依赖、数据库结构或服务端接口变更，可直接替换服务端程序并发布新版客户端。
